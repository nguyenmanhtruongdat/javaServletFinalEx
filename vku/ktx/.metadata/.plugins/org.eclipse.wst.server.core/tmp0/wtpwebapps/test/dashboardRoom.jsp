...
<body>
	...
	<main style="margin-top: 58px">
		<!-- Modal -->
		<div class="modal fade" id="modalRoomInfor" tabindex="-1"
			role="dialog" aria-labelledby="modalRoomInfor" aria-hidden="true">
		...
		<table class="table table-striped">
			<thead>
				<tr>
					...
					<th scope="col">Tiền điện tháng</th>
					<th colspan="2">Tình trạng thu</th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${listR}" var="x">
					<tr>
						<td><a href="#" id="room" data-toggle="modal"
							data-target="#modalRoomInfor" onclick="showRoomDetail(this)">${x.maphong}</a></td>
						<td>${x.songuoi}</td>
						<td>${x.tiendien}</td>
						<td><c:if test="${x.dathu==1}">Đã thu</c:if> <c:if
								test="${x.dathu==0}">Chưa thu</c:if></td>
						<td>
					</tr>
	...
	function showMess(id) {
	    ...
	    if (option === true) {
	      window.location.href = "deleteRoom?maphong=" + id;
	    }
	  }
	
	function showUpdate(id) {
		window.location.href = "updateRoom?maphong=" + id;
	}
	
    function showRoomDetail(x) {
    	var maphongSearch =x.innerText;
