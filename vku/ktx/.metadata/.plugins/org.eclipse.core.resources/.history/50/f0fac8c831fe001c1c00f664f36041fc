									...
									<form id="contact-form" role="form">
										...
										<div class="controls">
											...
											<div class="row">
												...
												<div class="col-md-6">
													<div class="form-group">
														<c:if test="${acc.isAdmin ==1 }">Admin<input
																type="radio" checked="checked" value="${acc.isAdmin}"
																name="isAdmin" class="form-control" id="isAdmin"
																required readonly="readonly">
															Người dùng<input type="radio" value="${acc.isUser}"
																name="isUser" class="form-control" id="isUser" required
																readonly="readonly">
														</c:if>
														<c:if test="${acc.isUser ==1 }">Người dùng<input
																type="radio" checked="checked" value="${acc.isUser}"
																name="isUser" class="form-control" id="isUser" required
																readonly="readonly">
															Admin<input type="radio" value="${acc.isAdmin}"
																name="isAdmin" class="form-control" id="isAdmin"
																required readonly="readonly">
														</c:if>
													</div>
