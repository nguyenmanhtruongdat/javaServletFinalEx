		...
		<tbody>
			<c:forEach items="${listD}" var="x">
				<tr>
					...
					<td>${x.maphong}</td>
					<td>
					<button class="btn btn-success" onclick="javascript:showCheck('${x.masv}')">Chấp nhận</button>
					<button type="button" class="btn btn-danger" onclick="showMess('${x.masv}')">Hủy</button>
					</td>
				</tr>
...
<script>	
	function showMess(id) {
	    var option = confirm("Bạn có muốn hủy không?");
	    if (option === true) {
	      window.location.href = "xoaYeuCauDKPhong?masv=" + id;
	    }
	  }
	
	function showCheck(id) {
		 var option = confirm("Bạn có muốn duyệt không?");
		    if (option === true) {
		window.location.href = "duyetphong?masv=" + id;
		    }
	}
</script>
    <script
